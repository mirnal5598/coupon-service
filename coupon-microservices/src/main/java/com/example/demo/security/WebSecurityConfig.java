...
//                .mvcMatchers(HttpMethod.GET, "/couponapi/coupons/{code:^[A-Z]*$}","/index", "/showGetCoupon", "/getCoupon","/couponDetails").anonymous()
                ...
                .mvcMatchers(HttpMethod.POST,"/login").permitAll().anyRequest().denyAll().and().csrf().disable().logout().logoutSuccessUrl("/");//.//{code:^[A-Z]*$}
    }
